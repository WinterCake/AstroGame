...
function isSpyReportComplete(report) {
...
function formatSpyReportDate(report) {
  if (report.timestamp) {
    const date = new Date(report.timestamp * 1000);
...
function applySpyHiddenFilter(reports, hiddenCoords) {
  const hidden = new Set(hiddenCoords ?? []);
  if (!hidden.size) return reports ?? [];
  return (reports ?? []).filter((report) => !hidden.has(report.coords));
}

return { sumDefenseExcludingMissiles, getEffectiveDefense, formatCompactNumber, buildVerdict, summarizeSpyPayload, parseSpyReportsHtml, detectMaxSpyPage, dedupeSpyReportsByCoords, isSpyReportComplete, mergeSpyReports, isSansDefense, isGrosButinSansDefense, isReportToday, filterSpyReports, buildSpyPayload, formatSpyReportDate, applySpyHiddenFilter };
})();
